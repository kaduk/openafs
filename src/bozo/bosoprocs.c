...
static int
waitproc(struct bnode *abnode, void *arock)
{
    afs_int32 code;
    opr_Assert(abnode->refCount > 0);

    code = bnode_WaitStatus(abnode, abnode->goal);
    return code;
}
...
SBOZO_WaitAll(struct rx_call *acall)
{
    ...
    code = bnode_ApplyInstance(waitproc, (void *) NULL, (void *) NULL);
...
SBOZO_ShutdownAll(struct rx_call *acall)
{
    ...
    code = bnode_ApplyInstance(sdproc, (void *) NULL, (void *) NULL);
...
SBOZO_RestartAll(struct rx_call *acall)
{
    ...
    code = bnode_ApplyInstance(sdproc, (void *) NULL,	/* start shutdown of all processes */
			       swproc, (void *) NULL,	/* wait for all done */
			       stproc, (void *) NULL,	/* start them up again */
			       (void *) NULL);
...
SBOZO_StartupAll(struct rx_call *acall)
{
    ...
    code = bnode_ApplyInstance(stproc, (void *) NULL, (void *) NULL);
		        ...
		        char **ainstance)
{
    ...
    **ainstance = 0;
    ...
    bnode_ApplyInstance(eifunc, &tdata, (void *) NULL);
    if (tdata.counter >= 0)
...
bozo_ShutdownAndExit(void *param)
{
    ...
    code = bnode_ApplyInstance(sdproc, (void *) NULL,	/* start shutdown of all processes */
			       swproc, (void *) NULL,	/* wait for shutdown to complete */
			       (void *) NULL);
